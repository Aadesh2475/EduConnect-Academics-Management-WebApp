    ...
    <div className="flex flex-col bg-[#000] h-[calc(100vh-4rem)]">
          ...
          <div className="border-r border-[#1C1C1C] flex flex-col h-full bg-[#161616]">
            <div className="p-4 border-b border-[#1C1C1C] space-y-4 shrink-0">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-white/5 text-[#444] hover:text-white transition-colors" onClick={openTeacherPicker}><Plus className="w-4 h-4" /></Button>
              </div>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#444] w-4 h-4" />
                <Input placeholder="Search messages..." value={searchQuery} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchQuery(e.target.value)} className="pl-9 bg-black border-[#1C1C1C] h-10 text-sm focus:border-white/40 transition-all text-white" />
              </div>
            </div>
              ...
              <div className="p-2 space-y-1">
                {loading ? (
                  ...
                  <button key={room.id} onClick={() => setSelectedRoom(room)}
                    className={`w-full group flex items-center gap-3 p-3 rounded-xl transition-all duration-300 text-left ${selectedRoom?.id === room.id ? "bg-white/10 border border-white/20 shadow-[0_0_20px_rgba(255,255,255,0.05)]" : "hover:bg-white/5 border border-transparent"}`}>
                    {room.type === "DIRECT"
                      ? <Avatar className="h-10 w-10 border border-[#1C1C1C]"><AvatarImage src={room.members[0]?.image || ""} /><AvatarFallback className="text-xs bg-white text-black font-bold">{getInitials(room.name)}</AvatarFallback></Avatar>
                      : <div className={`w-10 h-10 rounded-xl flex items-center justify-center border border-[#1C1C1C] ${room.type === "CLASS" ? "bg-[#3b82f6]/10" : "bg-white/10"}`}><Users className={`w-5 h-5 ${room.type === "CLASS" ? "text-[#3b82f6]" : "text-white"}`} /></div>
                    }
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between gap-1 mt-0.5">
                        ...
                        {room.unreadCount > 0 && <Badge className="h-4 min-w-[16px] px-1 text-[8px] shrink-0 bg-white text-black border-0 font-bold">{room.unreadCount}</Badge>}
                      </div>
                    </div>
                  </button>
                ))}
            ...
            <div className="border-t border-[#1C1C1C] p-2 shrink-0 space-y-1">
              <button onClick={openTeacherPicker} className="w-full flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/5 transition-all text-sm font-medium text-[#555] hover:text-white group">
                <PenSquare className="w-4 h-4 shrink-0 transition-colors group-hover:text-white" /><span>New Chat</span>
              </button>
              <button onClick={() => setHelpOpen(true)} className="w-full flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/5 transition-all text-sm font-medium text-[#555] hover:text-white group">
                <HelpCircle className="w-4 h-4 shrink-0 transition-colors group-hover:text-white" /><span>Support</span>
              </button>
              <button onClick={() => setPrivacyOpen(true)} className="w-full flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/5 transition-all text-sm font-medium text-[#555] hover:text-white group">
          ...
          <div className="flex flex-col h-full bg-[#000]">
            {selectedRoom ? (
                ...
                <div className="px-6 py-4 border-b border-[#1C1C1C] flex items-center justify-between shrink-0 bg-[#0A0A0A]/50 backdrop-blur-md z-10">
                  <div className="flex items-center gap-4">
                    {selectedRoom.type === "DIRECT"
                      ? <Avatar className="h-10 w-10 border border-[#1C1C1C]"><AvatarImage src={selectedRoom.members[0]?.image || ""} /><AvatarFallback className="text-xs bg-white text-black font-bold">{getInitials(selectedRoom.name)}</AvatarFallback></Avatar>
                      : <div className={`w-10 h-10 rounded-xl flex items-center justify-center border border-[#1C1C1C] ${selectedRoom.type === "CLASS" ? "bg-[#3b82f6]/10" : "bg-white/10"}`}><Users className={`w-5 h-5 ${selectedRoom.type === "CLASS" ? "text-[#3b82f6]" : "text-white"}`} /></div>}
                    <div>
                ...
                <ScrollArea className="flex-1 p-6 relative bg-[radial-gradient(circle,_#1C1C1C_1px,_transparent_1px)] bg-[length:32px_32px]">
                  {msgLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
                        <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center animate-pulse"><Lock className="w-8 h-8 text-white/40" /></div>
                      ...
                      {messages.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.isMe ? "justify-end" : "justify-start"} group perspective-1000`}>
                          <div className={`flex items-end gap-3 max-w-[80%] ${msg.isMe ? "flex-row-reverse" : ""}`}>
                            {!msg.isMe && <Avatar className="h-8 w-8 border border-[#1C1C1C] shrink-0 mb-1"><AvatarImage src={msg.senderImage || ""} /><AvatarFallback className="text-[10px] bg-white text-black font-bold">{getInitials(msg.senderName)}</AvatarFallback></Avatar>}
                            <div className="flex flex-col gap-1">
                                {!msg.isMe && selectedRoom.type !== "DIRECT" && <p className="text-[9px] font-semibold text-[#555] ml-2 mb-0.5">{msg.senderName}</p>}
                                <div className={`relative px-4 py-2.5 rounded-2xl text-sm leading-normal shadow-none border ${msg.isMe
                                ? "bg-white text-black border-white rounded-br-none"
                                : "bg-[#161616] border-[#1C1C1C] text-[#CCC] rounded-bl-none"
                                }`}>
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 group-hover:opacity-100 transition-all hover:bg-white/5 text-[#444] hover:text-white shrink-0 mb-1"><MoreVertical className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger>
                              <DropdownMenuContent align={msg.isMe ? "end" : "start"} className="bg-[#0A0A0A] border-[#1C1C1C] min-w-[120px]">
                                <DropdownMenuItem className="text-xs font-semibold focus:bg-white/5" onClick={() => handleCopy(msg.content)}><Copy className="w-3.5 h-3.5 mr-2 text-white" />Copy</DropdownMenuItem>
                ...
                <div className="p-4 border-t border-[#1C1C1C] bg-[#0A0A0A]/50 backdrop-blur-md">
                  <div className="flex gap-3 max-w-5xl mx-auto">
                    <Input 
                        ...
                        className="flex-1 bg-[#000] border-[#1C1C1C] focus:border-white/40 text-sm text-white h-12 px-4 rounded-xl transition-all" 
                        disabled={sending} 
                    />
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#000] relative overflow-hidden">
                <div className="absolute inset-0 opacity-10 bg-[radial-gradient(#ffffff_0.5px,_transparent_0.5px)] bg-[length:24px_24px]" />
                <div className="text-center space-y-6 max-w-sm relative z-10">
                  <div className="w-24 h-24 rounded-[2rem] bg-white/5 border border-white/10 flex items-center justify-center mx-auto shadow-[0_0_50px_rgba(255,255,255,0.05)] transform hover:rotate-6 transition-all duration-500 group">
                    <MessageSquare className="w-10 h-10 text-white/40 group-hover:text-white transition-colors" />
                  </div>
      ...
      <Dialog open={newMsgOpen} onOpenChange={setNewMsgOpen}>
        <DialogContent className="bg-[#0A0A0A] border-[#1C1C1C] p-0 overflow-hidden max-w-md">
            <div className="p-6 border-b border-[#1C1C1C] bg-white/5 flex items-center justify-between">
                <div>
            ...
            <div className="p-6 space-y-4">
                <div className="relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#444] w-4 h-4" />
                    <Input placeholder="Search teachers..." value={teacherSearch} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTeacherSearch(e.target.value)} className="pl-10 bg-black border-[#1C1C1C] h-11 text-sm focus:border-white/40 transition-all text-white" />
                </div>
                    ...
                    <div className="p-2 space-y-1">
                        {teachersLoading ? (
                            ...
                            <button key={t.id} onClick={() => startChat(t.id)} className="w-full flex items-center gap-4 p-3 rounded-xl hover:bg-white/5 transition-all text-left group">
                                <Avatar className="h-11 w-11 border border-[#1C1C1C]"><AvatarImage src={t.image || ""} /><AvatarFallback className="text-xs bg-white text-black font-bold">{getInitials(t.name)}</AvatarFallback></Avatar>
                                <div className="flex-1">
                                    <p className="text-sm font-semibold text-white group-hover:text-white transition-colors">{t.name}</p>
                                    <p className="text-[10px] text-[#555] font-medium">{t.classes.join(" · ")}</p>
                                </div>
                                <ChevronRight className="w-4 h-4 text-[#444] group-hover:text-white transition-all" />
                            </button>
                        ))}
                    </div>
                </ScrollArea>
                ...
                <div className="p-6 space-y-6 pr-10">
                    {HELP_ITEMS.map(({ q, a }) => (
                        <div key={q} className="group space-y-2">
                            <p className="text-xs font-semibold text-white flex items-center gap-2 tracking-tight"><ChevronRight className="w-3 h-3 transition-transform group-hover:translate-x-1" /> {q}</p>
      ...
      <Dialog open={privacyOpen} onOpenChange={setPrivacyOpen}>
        <DialogContent className="bg-[#0A0A0A] border-[#1C1C1C] p-0 overflow-hidden max-w-md">
            <div className="p-8 text-center space-y-6">
                <div className="w-16 h-16 rounded-full bg-white/10 border border-white/20 flex items-center justify-center mx-auto shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                ...
                <div className="space-y-4">
                    ...
                    <div className="p-4 rounded-2xl bg-[#161616] border border-[#1C1C1C] text-left space-y-3">
                        <div className="flex gap-3">
                            <div className="w-6 h-6 rounded-lg bg-black/50 border border-[#1C1C1C] flex items-center justify-center shrink-0"><Lock className="w-3 h-3 text-white" /></div>
