...
import {
    ...
    ThumbsUp, ThumbsDown, MessageSquare, Users, BarChart3, AlertTriangle,
    TrendingDown, BookOpen, ChevronLeft, Lightbulb, Eye,
} from "lucide-react"
...
// ─── Quick prompts ────────────────────────────────────────────────────────────
const QUICK_PROMPTS = [
    { label: "Show me at-risk students", icon: AlertTriangle },
    { label: "Who has the lowest attendance?", icon: TrendingDown },
    { label: "Class performance summary", icon: BarChart3 },
    { label: "Students with no recent submissions", icon: Eye },
    { label: "How can I improve engagement?", icon: Users },
    { label: "Suggest effective teaching strategies", icon: Lightbulb },
]
    ...
    return (
        <div className="space-y-1 text-sm leading-relaxed">
            {lines.map((line, i) => {
                ...
                if (line.startsWith("## "))
                    return <h3 key={i} className="font-semibold text-foreground text-base mt-3 mb-1">{line.slice(3)}</h3>
                if (line.startsWith("### "))
                    return <h4 key={i} className="font-semibold text-foreground/90 mt-2 mb-0.5">{line.slice(4)}</h4>
                if (line.startsWith("- ") || line.startsWith("• "))
                    return (
                        <div key={i} className="flex gap-2 items-start">
                            <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-primary shrink-0" />
                            <span className="text-foreground/80">{inlineBold(line.slice(2))}</span>
                        </div>
                    )
                ...
                if (numMatch)
                    return (
                        <div key={i} className="flex gap-2 items-start">
                            <span className="font-semibold text-primary min-w-[1.2rem] text-right shrink-0">{numMatch[1]}.</span>
                            <span className="text-foreground/80">{inlineBold(numMatch[2])}</span>
                        </div>
                    )
                return <p key={i} className="text-foreground/80">{inlineBold(line)}</p>
            })}
        </div>
    )
}
...
export default function TeacherAIAssistant() {
    ...
    async function fetchSessions() {
        ...
        try {
            ...
            if (data.success) {
                setSessions(data.data.filter((s: ChatSession & { role: string }) => s.role === "TEACHER"))
            }
        } catch { /* silent */ }
        ...
        async (text?: string) => {
            ...
            try {
                const res = await fetch("/api/chatbot/sessions", {
                    ...
                    body: JSON.stringify({ message: msg, sessionId: activeSessionId, userRole: "TEACHER" }),
                })
                const data = await res.json()
                if (!res.ok || !data.success) throw new Error(data.error || "Failed")

                const { sessionId, response, messageId } = data.data
                if (!activeSessionId) { setActiveSessionId(sessionId); fetchSessions() }

                setMessages((prev) => [...prev, { id: messageId, role: "assistant", content: response, createdAt: new Date().toISOString() }])
            } catch (err) {
                toast({ title: "AI Error", description: err instanceof Error ? err.message : "Something went wrong", variant: "destructive" })
    ...
    // Handle automated initial message from reports
    useEffect(() => {
        const message = searchParams.get("message")
        if (message && !initialMessageProcessed.current && sessions.length >= 0 && !loadingSessions) {
            initialMessageProcessed.current = true
            try {
                // searchParams.get() is already decoded by Next.js,
                // but we'll try to decode just in case it was double encoded.
                // If it fails, we use the message as is.
                const decodedMessage = message.includes('%') ? decodeURIComponent(message) : message
    ...
    }, [searchParams, sessions, loadingSessions, handleSend])
                ...
                {sidebarOpen && (
                    ...
                    >
                        <div className="flex items-center gap-2 p-3 border-b border-[#1C1C1C]">
                            <Button onClick={newChat} size="sm" className="flex-1 h-8 text-[11px] font-semibold bg-white hover:bg-white/90 text-black gap-1.5 rounded-lg border-none shadow-sm">
                        ...
                        <ScrollArea className="flex-1 px-2 py-2">
                            {loadingSessions ? (
                                <div className="space-y-2 p-2">
                                    {[1, 2, 3].map((i) => <div key={i} className="h-10 rounded-lg bg-white/5 animate-pulse border border-[#1C1C1C]" />)}
                                </div>
                            ) : sessions.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-32 text-center px-4">
                                    ...
                                    <p className="text-[10px] font-bold text-[#444] uppercase tracking-wider">No previous chats</p>
                                </div>
                            ) : (
                                    ...
                                    {sessions.map((s) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className={cn("text-xs font-semibold truncate", activeSessionId === s.id ? "text-[#00E599]" : "text-white/80")}>{s.title ?? "New Chat"}</p>
                                                    <p className="text-[9px] text-[#444] font-medium mt-0.5">{formatDistanceToNow(new Date(s.updatedAt), { addSuffix: true })}</p>
                                                </div>
                                            <button
                ...
                <div className="flex items-center gap-4 px-6 py-4 border-b border-[#1C1C1C] shrink-0 bg-[#0A0A0A]/50 backdrop-blur-xl z-20">
                    ...
                    <div className="flex items-center gap-3.5 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                            <h1 className="text-sm font-bold text-white leading-none tracking-tight">Teacher AI Assistant</h1>
                        ...
                        {isNewChat && (
                            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="pt-12">
                                <div className="text-center mb-12">
                                    ...
                                    <p className="text-sm text-[#555] max-w-md mx-auto font-medium">Ask questions about your classes and students, or get teaching tips.</p>
                                </div>
                                    ...
                                    <div className={cn("flex flex-col gap-1.5 max-w-[85%]", msg.role === "user" && "items-end")}>
                                        ...
                                        {msg.role === "assistant" && (
                                            <div className="flex items-center gap-1 pl-1">
                                                <button onClick={() => copyMessage(msg.id, msg.content)} className="p-2 rounded-lg text-[#444] hover:text-[#00E599] hover:bg-white/5 transition-all" title="Copy report">
                                                ...
                                                <button
                                                    ...
                                                    className={cn("p-2 rounded-lg transition-all", msg.feedback === "up" ? "text-[#00E599] bg-[#00E599]/10" : "text-[#444] hover:text-[#00E599] hover:bg-white/5")}
                                                    title="Valuable input"
                                                ><ThumbsUp className="w-3.5 h-3.5" /></button>
                                                <button
                                                    onClick={() => saveFeedback(msg.id, msg.feedback === "down" ? null : "down")}
                                                    className={cn("p-2 rounded-lg transition-all", msg.feedback === "down" ? "text-[#F87171] bg-[#F87171]/10" : "text-[#444] hover:text-[#F87171] hover:bg-white/5")}
                                ...
                                <div className="bg-[#161616] border border-[#1C1C1C] rounded-2xl rounded-bl-none px-5 py-3.5 shadow-xl">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex gap-1.5">
                                            {[0, 1, 2].map((i) => (
                                                <motion.div key={i} className="w-1.5 h-1.5 rounded-full bg-[#00E599]/40"
                ...
                <div className="p-6 border-t border-[#1C1C1C] bg-[#0A0A0A]/50 backdrop-blur-md z-30">
                    <div className="w-full max-w-4xl mx-auto space-y-4">
                        <div className="flex items-end gap-3 bg-black border border-[#1C1C1C] rounded-2xl px-4 py-3 focus-within:border-[#00E599]/40 transition-all shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
                            <Textarea
                                placeholder="Transmit query to academic intelligence..."
                                value={input}
                        ...
                        <div className="flex items-center justify-center gap-3 opacity-20 group cursor-default">
                             ...
                             <p className="text-[10px] text-white font-bold uppercase tracking-[0.3em] whitespace-nowrap">
                                AI Assistant
                             </p>
